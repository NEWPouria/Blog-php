...
class Users
{
    ...
    public static function FetchUserInfoBYID($UserID)
    {
        ...
        $FetchUserInfo_sqlResult = $WillPrepare_FetchUserInfo_sql->get_result();
        $UserInfo = $FetchUserInfo_sqlResult->fetch_assoc();
        $WillPrepare_FetchUserInfo_sql->close();
        return $UserInfo;
        //این تابع نام کاربری و تاریخ عضویت را میدهد
        //دقت کن که تو در خروجی این تابع فقط یک متغییر برگردوندی
        //پس در صفحه هرجا که میخواهی ازش استفاده کنی هم اول بریز تو یک متغییر آرایه ای
    }
    public function UpdateUserInfo($UserID, $UserName, $Email, $Phone)
    {
        
        $Data = new DataBase();
        $UpdateUserInfo_sql = "UPDATE users SET UserName = ? , Email = ? , Phone = ? WHERE UserID=?;";
        $WillPrepare_UpdateUserInfo_sql = $Data->myprepare($UpdateUserInfo_sql);
        $WillPrepare_UpdateUserInfo_sql->bind_param("sssi", $UserName, $Email, $Phone, $UserID);
        $WillPrepare_UpdateUserInfo_sql->execute();
        $WillPrepare_UpdateUserInfo_sql->close();
    }
}
